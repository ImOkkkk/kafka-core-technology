...
控制器组件(Controller)，在Zookeeper的帮助下管理和协调整个Kafka集群。正常运转的Kafka集群，有且仅有一个控制器。
...
2. 分区重分配
3. Preferred领导者选举
4. 集群成员管理(新增Broker、Broker主动关闭/宕机)
5. 数据服务
...
- 所有主题信息，包括具体的分区信息。如领导者副本是谁，ISR集合中有哪些副本等；
- 所有Broker信息，包括当前都有哪些正在运行的Broker，哪些正在关闭的Boker等；
- 所有设计运维任务的分区，包括当前正在进行Preferred领导者选举以及分区重分配的分区列表。

这些数据在Zookeeper中也保存了一份，当控制器初始化时，都会从Zookeeper中读取对应的元数据并填充到自己的缓存中。有了这些数据，控制器就可以通过向其它Broker发送请求的方式将这些数据同步到其它Boker上。

**控制器故障转移(Failover)**

当运行中的控制器突然意外终止时，Kafka能够快速的感知到，并立即启用备用控制器替代之前失败的控制器。

![](img/控制器故障转移.png)

**控制器内部设计**

- 之前是多线程设计，0.11版本之后改为单线程加事件队列的方案，避免多线程之间的线程同步开销；
- 同步操作Zookeeper改为异步；
- 2.2版本之后，Kafka将控制器发送的请求与普通数据类请求分开，实现控制器请求单独处理(优先级更高)的逻辑

如果控制器组件出现问题，如主题无法删除，或重分区hang住了，不需要重启Kafka，去 ZooKeeper执行命令**rmr /controller**，手动删除/controller节点，既可以引发控制器的重选举，又可以避免重启Broker导致的消息处理中断。

### 4.5 高水位&Leader Epoch
...
//修改主题基本参数
bin/kafka-config.sh  
	--zookeeper-server broker_host:port 
	--entity-type topics 
	--entity-name <topic_name> 
	--alter 
	--add-config max.message.bytes=10485760
	
//修改主题限速(主题test，该主题各个分区的leader副本和Follower副本在执行副本同步时，不得占用超过100MBps的带宽)
...
内部主题 __ consumer_offsets 和  __ transaction_state(默认50个分区)
...
- 主题删除失败
  ...
  1. 手动删除Zookeeper节点/admin/delete_topics下待删除主题为名的znode；
  2. 手动删除该主题在磁盘上的分区目录；
  3. 在Zookeeper中执行rmr/controller，触发Controller重新选举，刷新COntroller缓存。
  
- __consumer_offsets占用太多的磁盘

  kafka-log-cleaner-thread前缀的线程挂掉，无法及时清理此内部主题。重启相应的Broker。

### 5.2 Kafka动态配置

**Dynamic Update Mode**

- read-only：重启Broker，命令才会生效；
- per-broker：动态参数，修改之后，只会在对应的Broker上生效；
- cluster-wide：动态参数，修改之后，会在整个集群范围内生效。
...
2. num.io.threads和num.network.threads
   ...
   当 Kafka Broker 入站流量激增时，会造成 Broker 端请求积压。有了动态参数，能够动态增加网络线程数和 I/O 线程数，快速消耗一些积压。当突发流量过去后，我们也能将线程数调整回来，减少对资源的浪费。

3. 与 SSL 相关的参数

   ssl.keystore.type、ssl.keystore.location、ssl.keystore.password 和ssl.key.password。阶段性地调整这组参数，有利于增加安全性。

4. num.replica.fetchers

   Follower副本拉取速度慢，增加改参数值，确保由足够的线程可以执行Follower副本向Leader副本的拉取。

### 5.3 重设消费者组位移

Kafka，基于日志结构（log-based）的消息引擎，消费者在消费消息时，仅仅是从磁盘文件上读取数据而已，是只读的操作，因此消费者不会删除消息数据。同时，由于位移数据是由消费者控制的，因此它能够很容易地修改位移的值，实现重复消费历史数据的功能。

**重设位移策略**

1. 位移维度；
2. 时间维度；给定时间，让消费者把位移调整成大于该时间的最小位移；给定时间间隔，如30分钟，让消费者将位移调回30分钟之前的位移值。

![](img/image-20220902094613448.png)

- 重新消费主题的所有消息，可以使用Earliest策略；
- 跳过所有历史消息，从最新消息处开始消费，可以使用Latest策略；
- 消费者程序在处理某条错误消息时，手动地“跳过”此消息的处理，可以使用Specified-Offset策略；
- 重新消费昨天的数据，可以使用DateTime策略。

重设消费者组位移的方式：

1. 通过消费者API来实现(Java API 的方式来实现重设策略的主要入口方法，就是 seek 方法)；
2. 通过kafka-consumer-groups命令行脚本实现(0.11 及 0.11 版本之后，kafka-consumer-groups脚本)。

### 5.4 Kafka Admin Client

**功能**

1. 主题管理
2. 权限管理
3. 配置参数管理：Kafka 各种资源(Broker、主题、用户、Client-id)的参数设置、详情查询
4. 副本日志管理
5. 分区管理
...
AdminClient是双线程设计：前端主线程和后端I/O线程

- 前端线程：将用户要执行的操作转换成对应的请求，然后再将请求发送到后端I/O线程的队列中；
- 后端I/O线程：从队列中读取相应的请求，然后发送对应的Broker节点上，之后把执行结果保存起来，以便前端线程的获取。

![](img/image-20221104170414646.png)

3个队列：新请求队列的线程安全是由Java的monitor锁来保证的，为了确定前端线程不会因为monitor锁被阻塞，后端I/O会定期的将新请求队列中的所有Call实例全部搬移到待发送请求队列中进行处理。待发送请求队列和处理中请求队列只由后端I/O线程处理，因此无需任何锁机制保证线程安全。
